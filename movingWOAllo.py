...
output.printProgress(0,range(10)) #Print progress on console
rbsAssignedToCU, cuAssignedToRB,r, alloc, rCU, rD2D = allocation.testingResourceAllocation(Nc, Nd, Nrb, Pd, pMax, bw, N0, tSNR, distance, cellUsers, d2dT, d2dR, time)
...
for i in range(1,9):
		...
		rCU, rD2D = allocation.throughput(rbsAssignedToCU, cuAssignedToRB,r, alloc, Nc, Nd, Nrb, Pd, pMax, bw, N0, tSNR, distance, cellUsers, d2dT, d2dR, time)
		cellThroughput = np.mean(rCU) #mean Throughput for all CUs
		d2dTrhoughput = np.mean(rD2D) #mean Throughput for all D2D pairs
		cellRates.append(cellThroughput) #add Cell user throughput to the list
		d2dsRates.append(d2dTrhoughput) #add D2D pairs throughput to the list
...
output.printProgress(9,range(10)) #Print progress on console
rbsAssignedToCU, cuAssignedToRB,r, alloc, rCU, rD2D = allocation.testingResourceAllocation(Nc, Nd, Nrb, Pd, pMax, bw, N0, tSNR, distance, cellUsers, d2dT, d2dR, time)
cellThroughput = np.mean(rCU) #mean Throughput for all CUs
d2dTrhoughput = np.mean(rD2D) #mean Throughput for all D2D pairs
cellRates.append(cellThroughput) #add Cell user throughput to the list
